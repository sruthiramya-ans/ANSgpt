Attribute VB_Name = "ANSgpt_Weak"
'Create LPILE Files
Sub ANSgptCreatorWeak() ' will eventually need inputs for file naming conventions as we batch runs
    ...
    Dim L As Double, laynum As Integer, i As Integer, j As Integer, k As Integer, depth As Double, pmult As Double, ymult As Double
    ...
    Do While True
        If IsExeRunning("LPile2019.exe") = False Then
            Exit Do
        Else 'Add check for Lpile2022.exe
    ...
    L = Range("Pile.Embed").Value2 + Range("Pile.Reveal").Value2
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    ...
    If arrDepthBot(laynum) < L + 1 Then
        arrDepthBot(laynum) = L + 1
    End If
...
'   Create "LPile" folder in current workbook directory
    FolderSpec = ThisWorkbook.Path & "\LPile"
    folderName = Dir(FolderSpec, vbDirectory)
    If folderName = "" Then
        MkDir FolderSpec
    End If

'   Set LPile file path/names
    'PLACEHOLDER TO enter load case / pile specific naming convention
    fileSpec = ThisWorkbook.Path & "\LPile\" & [Project.Name].Value2 & "- ANSgpt_Weak.lp11d"
    fileName = Dir(fileSpec)

'   Check to see if LPile files already exist
    If fileName <> "" Then
        If MsgBox("Overwrite existing LPile files?", vbQuestion + vbYesNo, "Existing Files") = vbNo Then
            Exit Sub
        Else
            Kill ThisWorkbook.Path & "\LPile\" & [Project.Name].Value2 & "- ANSgpt_Weak.lp11d"
        End If
    End If
...
'PLACEHOLDER to have multiple pile sections for atmospheric vs soil corrosion, collars, etc
    data3 = _
    "SECTIONS" & vbCrLf & _
...
Sub ANSgptOutputWeak(fileSpec As String)
...
'    fileSpec = ThisWorkbook.Path & "\LPile\" & [Project.Name].Value2 & "- ANSgpt_Weak.lp12o"
    ...
    [lpile.output2.weak].ClearContents
    startCol = [lpile.output.weak].Column
    startRow = [lpile.output.weak].row
